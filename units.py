...
def metal_unit_system(constants: Dict=constants_CONDATA_2014):
  ...
  Returns:
    Dictonary of conversion factors
  """
  logging.warning('This unit system will work only if the potential parameters are in '
    'eV, Angstroms, grams/mol and proton charge for energy, distance, mass and charge.')
  ...
  pascal = (eV * Ang_conv_factor * Ang_conv_factor * Ang_conv_factor / constants['_e'])  # 1 / pressure_conv_factor # J/m^3 i.e pascal
  # eV/A^{3} -> bar
  bar = 1e5 * pascal
...
def real_unit_system(constants: Dict=constants_CONDATA_2014):
  ...
  """
  logging.warning('This unit system will work only if the potential parameters are in '
                'kcal/mol, Angstroms, grams/mol and proton charge for energy, distance, mass and charge.')
  ...
  Ang_conv_factor = 1e-10 # Meter
  second = jnp.sqrt(Kcal_per_mol / (constants['_amu'] * Ang_conv_factor * Ang_conv_factor * constants['_Nav'] * 1e-3 / 4.184)) # Kg, m, J
  femtosecond = 1e-15 * second # femtosecond

  kB = constants['_k'] * constants['_Nav'] / 4.184 / 1e3 # Boltzmann constant in kcal/mol K
  # Kelvin = 1 / kB
  
  pascal = (Ang_conv_factor * Ang_conv_factor * Ang_conv_factor) * (constants['_Nav'] * 1e-3 / 4.184)  # 1 / pressure_conv_factor # J/m^3 i.e pascal
  # kcal/mol A^{3} -> bar
  # Note that LAMMPS uses atm
  bar = 1e5 * pascal
